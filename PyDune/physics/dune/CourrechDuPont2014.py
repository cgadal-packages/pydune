...
import numpy as np
from PyDune.General import Vector_average, cosd, sind
...
def Resultant_flux_perp_crest_at_crest(alpha, theta, Q0, gamma=1.6, axis=-1):
    ...
    """
    RDD, RDP = Resultant_flux_at_crest(alpha, theta, Q0, gamma=1.6, axis=axis)
    alpha_squeezed = np.squeeze(alpha, axis=axis)
    return RDP*(cosd(alpha_squeezed + 90)*cosd(RDD) + sind(alpha_squeezed + 90)*sind(RDD))


def Elongation_direction(theta, Q0, gamma=1.6, alpha_bins=np.linspace(0, 360, 361), **kwargs):
    ...
    **kwargs :
    ...
    th_expanded, N_expanded = np.expand_dims(theta, 0), np.expand_dims(Q0, 0)
    #
    Alpha_F = alpha_expanded[np.argmin(np.abs(Resultant_flux_perp_crest_at_crest(alpha_expanded, th_expanded, N_expanded, gamma=gamma, **kwargs)), axis=0)]
...
def Bed_Instability_Orientation(theta, Q0, gamma=1.6, alpha_bins=np.linspace(0, 360, 361), **kwargs):
    ...
    **kwargs :
    ...
    th_expanded, N_expanded = np.expand_dims(theta, 0), np.expand_dims(Q0, 0)
    #
    G_rate = Growth_rate(alpha_expanded, th_expanded, N_expanded, gamma=gamma, **kwargs)
